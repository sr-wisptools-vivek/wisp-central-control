		...
		'click #signup-link': function(event) {
			...
			if (onCreateFn){
				...
				if (username !== null) {
					...
					if (document.getElementById('login-username') && document.getElementById('login-email')) {
	...
	Template._loginButtonsLoggedOutPasswordService.helpers({
		fields: function() {
			var loginFields = [{
				...
				visible: function() {
					return _.contains(
						["USERNAME_AND_EMAIL_CONFIRM", "USERNAME_AND_EMAIL", "USERNAME_AND_OPTIONAL_EMAIL", "EMAIL_AND_LEGACY_USERNAME"],
			...
			var signupFields = [{
				...
				visible: function() {
					return _.contains(
						["USERNAME_AND_EMAIL_CONFIRM", "USERNAME_AND_EMAIL", "EMAIL_ONLY", "EMAIL_AND_LEGACY_USERNAME"],
						Accounts.ui._passwordSignupFields());
				}
			}, {
				fieldName: 'email',
